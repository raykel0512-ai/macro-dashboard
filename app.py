...
def ai_commentary(context_data, focus):
    try:
        ...
        if not result or not result.strip():
            ...
            return f"⚠️ AI 응답 비어있음 (finish_reason: {finish_reason})\n\n[보낸 데이터]\n{context_data}"
        return result
    except Exception as e:
...
def load_financials(ticker, market):
    ...
    try:
        if market in ("US", "EU"):
            ...
            result["key_metrics"] = {"note": "한국 종목 - Supabase 캐시 사용"}
    except Exception as e:
        st.warning(f"재무 데이터 로딩 실패: {e}")
    return result

# ============================================
# 펀더멘털 데이터 - 한국 (Supabase 캐시)
# ============================================
@st.cache_data(ttl=3600)
...
def ai_overall_score(ticker, name, market, metrics_text):
    try:
        client = get_openai()
        prompt = f"""[{name} ({ticker}, {market}) 펀더멘털 종합 평가]

데이터:
{metrics_text}

다음 4개 영역을 각 100점 만점으로 채점하고 근거 제시. 마지막에 종합 점수와 한줄평.
...
def add_to_watchlist(ticker, name, market):
    try:
        sb.table("watchlist").insert({"ticker": ticker, "name": name, "market": market}).execute()
        return True
    except Exception as e:
        st.error(f"추가 실패: {e}")
        return False
...
def add_note(ticker, market, title, content, target_price, stop_loss, sentiment):
    try:
        sb.table("stock_notes").insert({
            "ticker": ticker, "market": market, "title": title, "content": content,
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    "🏠 Overview", "🚨 Recession Watch", "🏦 Fed Watch",
    "🇰🇷 Korea", "💰 Assets", "🔍 Explorer", "📈 Stocks"
])
    ...
    if st.button("🤖 Overview AI 해설 보기", key="overview_ai"):
        with st.spinner("GPT-5 mini가 분석 중..."):
            commentary = ai_commentary(
                overview_summary,
                "미국 거시경제 현황을 종합 진단해줘. 인플레이션, 고용, 금리, 시장 변동성을 묶어서."
            )
            st.info(commentary)
...
with tab7:
    ...
    with st.expander("➕ 종목 추가/삭제"):
        st.caption("💡 EU 종목: 거래소 접미사 포함 (예: AIR.PA, ASML.AS, BA.L, SAP.DE)")
        col_a, col_b, col_c, col_d = st.columns([2, 2, 1, 1])
        with col_a:
            new_ticker = st.text_input("티커", placeholder="AAPL / 005930 / AIR.PA", key="new_ticker")
        with col_b:
            new_name = st.text_input("종목명", placeholder="Apple / 삼성전자 / Airbus", key="new_name")
        with col_c:
            new_market = st.selectbox(
                "시장", ["US", "KR", "EU"],
                key="new_market",
                help="EU: 거래소 접미사 필요 (.PA .DE .AS .L .MI .SW)"
            )
        with col_d:
            st.write("")
            st.write("")
    ...
    st.divider()
    ticker_options = {
        f"[{w['market']}] {w['ticker']} - {w['name']}": (w['ticker'], w['market'])
        for w in watchlist
    }
    ...
    with fund_tab1:
        if st.button("📊 재무 분석 시작", key="fin_btn"):
            ...
            if selected_market in ("US", "EU"):
                ...
                if metrics.get("매출 (TTM)"):
                    st.write(f"**📌 주요 재무 지표 (TTM, 통화: {currency})**")

                    c1, c2, c3, c4 = st.columns(4)
                    c1.metric("시가총액", format_large_number(metrics.get("시가총액"), currency))
                    c2.metric("매출 (TTM)", format_large_number(metrics.get("매출 (TTM)"), currency))
                    c3.metric("순이익 (TTM)", format_large_number(metrics.get("순이익 (TTM)"), currency))
                            ...
                            if "Total Revenue" in income.index:
                                ...
                                if len(revenue) > 0:
                                    ...
                                    fig_rev.add_trace(go.Bar(
                                        ...
                                        y=revenue.values / unit_div,
                                        marker_color='#1f77b4',
                                        text=[f"{symbol}{v/unit_div:.1f}{unit_label}" for v in revenue.values],
                                        textposition='outside'
                                    ))
                                    fig_rev.update_layout(
                                        title=f"연간 매출 추이 (Billion {currency})",
                    ...
                    with st.spinner("GPT-5 mini 분석 중..."):
                        ...
                        market_full = "유럽" if selected_market == "EU" else "미국"
                        analysis = ai_financial_analysis(
                            selected_ticker, info.get("name", selected_ticker), market_full,
                ...
                with st.spinner("Supabase에서 한국 재무 데이터 조회 중..."):
                    kr_data = load_korean_financials(selected_ticker)

                if "error" in kr_data:
                    st.error(kr_data["error"])
                else:
                    metrics = kr_data["key_metrics"]
                    st.write(f"**📌 주요 재무 지표 ({metrics.get('기준연도', '')})**")

                    c1, c2, c3 = st.columns(3)
                    c1.metric("매출액", format_large_number(metrics.get("매출액"), "KRW"))
                    c2.metric("영업이익", format_large_number(metrics.get("영업이익"), "KRW"))
                    c3.metric("당기순이익", format_large_number(metrics.get("당기순이익"), "KRW"))

                    c4, c5, c6 = st.columns(3)
                    c4.metric("영업이익률", f"{metrics.get('영업이익률'):.2f}%" if metrics.get("영업이익률") else "N/A")
                    c5.metric("순이익률", f"{metrics.get('순이익률'):.2f}%" if metrics.get("순이익률") else "N/A")
                    c6.metric("ROE", f"{metrics.get('ROE'):.2f}%" if metrics.get("ROE") else "N/A")

                    c7, c8, c9 = st.columns(3)
                    c7.metric("ROA", f"{metrics.get('ROA'):.2f}%" if metrics.get("ROA") else "N/A")
                    c8.metric("부채비율", f"{metrics.get('부채비율'):.0f}%" if metrics.get("부채비율") else "N/A")
                    c9.metric("자본총계", format_large_number(metrics.get("자본총계"), "KRW"))

                    if kr_data.get("income_history"):
                        hist = kr_data["income_history"]
                        years = [str(h["year"]) for h in hist]
                        revenues = [h["revenue"] / 1e12 if h["revenue"] else 0 for h in hist]
                        op_profits = [h["op_profit"] / 1e12 if h["op_profit"] else 0 for h in hist]
                        fig_kr = go.Figure()
                        fig_kr.add_trace(go.Bar(x=years, y=revenues, name="매출액 (조원)",
                                                marker_color='#1f77b4'))
                        fig_kr.add_trace(go.Bar(x=years, y=op_profits, name="영업이익 (조원)",
                                                marker_color='#ff7f0e'))
                        fig_kr.update_layout(title="연도별 실적 추이 (조원)",
                    ...
                    with st.spinner("GPT-5 mini 분석 중..."):
                        metrics_text = "\n".join([
                            f"- {k}: {v:,.0f}원" if isinstance(v, (int, float)) and k in ['매출액', '영업이익', '당기순이익', '자산총계', '자본총계', '부채총계']
                            else f"- {k}: {v:.2f}%" if isinstance(v, (int, float))
                            else f"- {k}: {v}"
                            for k, v in metrics.items() if v is not None
                        ])
                        analysis = ai_financial_analysis(
                            selected_ticker, info.get("name", selected_ticker), "한국",
                            metrics_text, "DART 공시 데이터"
                        )
                        st.info(analysis)
    ...
    # --- 🎯 종합 점수 ---
    with fund_tab5:
        st.caption("4개 영역(수익성/안정성/성장성/밸류에이션)을 각 100점 만점으로 채점")
